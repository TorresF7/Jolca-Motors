...
$inicio = $producto_por_pagina * ($pagina_actual -1);
...
function listarProductos(){ 
    ...
    try{
        ...
        $sqlProducto = "SELECT SQL_CALC_FOUND_ROWS *
                        ...
                        WHERE FechaInicio < NOW() AND NOW()<FechaCaduc AND Estado=1
                        ORDER BY IdProducto
...
// LISTAR PRODUCTOS
function listarCategorias($buscarProducto){ 
    include 'conectar.php';
    // Mostrar productos
    try{
        global $producto_por_pagina;
        global $inicio;
        $sqlProducto = "SELECT SQL_CALC_FOUND_ROWS *
                        FROM producto WHERE IdCategoria=$buscarProducto AND Estado=1
                        ORDER BY IdProducto
                        LIMIT $inicio, $producto_por_pagina";
...
function totalPaginas(){
    ...
    include 'conectar.php';
    global $producto_por_pagina;
    $sql = "SELECT COUNT(IdOferta) as total_filas FROM oferta";
...
// TOTAL PAGINAS CATEGORÍA
function totalPaginasCategoria($buscarProducto){

    include 'conectar.php';
    global $producto_por_pagina;
    $sql = "SELECT COUNT(IdProducto) as total_filas FROM producto WHERE IdCategoria=$buscarProducto";
